...
class HistoryModel extends CI_Model
{
    ...
    //     JOIN tbl_user ON tbl_history.username = tbl_user.username")->result();
    // }
    ...
    //     JOIN tbl_material ON tbl_history.material_number = tbl_material.material_number 
    //     JOIN tbl_user ON tbl_history.username = tbl_user.username WHERE tbl_history.datetime BETWEEN '$start' AND '$end'
    //  ";
    ...
    function getListHistory($postData = null)
    {
        ...
        $searchStart = $postData['searchStart'];
        $searchEnd = $postData['searchEnd'];
        // $searchName = $postData['searchName'];

        ## Search 
        $searchQuery = "";
        if ($searchValue != '') {
            $searchQuery = "(material_number ilike '%" . $searchValue . "%' OR username ilike '%" . $searchValue . "%' OR action ilike '%" . $searchValue . "%')";
        }

        if ($searchStart != '' && $searchEnd != '') {
            $searchQuery .= " (datetime between '" . $searchStart . "' and '" . $searchEnd . "' )";
        }
